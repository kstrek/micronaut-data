The default ID generation for Mongo is using `ObjectId` as an ID, there are only two supported types: the default `ObjectId` and a simple Java String which will have the hex value of the `ObjectId`.
