To get started with Micronaut Data Mongo add the following dependency to your annotation processor path:
...
snippet::example.Book[project-base="doc-examples/mongo-example", source="main"]
...
With Micronaut Data Mongo, you must manually implement an `update` method since the Mongo implementation doesn't include any dirty checking or persistence session notion. So you have to define explicit update methods for updates in your repository. For example:
...
Congratulations you have implemented your first Micronaut Data Mongo repository! Read on to find out more.
