...
class MongoManyToManySpec extends Specification implements MongoTestPropertyProvider {
    ...
    void "test build Student select with courses"() {
        ...
        then:
            q.query == '''[{$lookup:{from:'student_course',localField:'_id',foreignField:'m2m_student',pipeline:[{$lookup:{from:'m2m_course',localField:'m2m_course',foreignField:'_id',pipeline:[],as:'m2m_course'}},{$unwind:{path:'$m2m_course',preserveNullAndEmptyArrays:true}},{$replaceRoot:{newRoot:'$m2m_course'}}],as:'courses'}},{$match:{'_id':{$eq:{$qpidx:0}}}}]'''
    }
    ...
    void "test build Student select with ratings"() {
        when:
            ...
            def q = encoder.buildQuery(queryModel.idEq(new QueryParameter("id")))
        then:
            q.query == '''[{$lookup:{from:'m2m_course_rating',localField:'_id',foreignField:'student._id',pipeline:[],as:'ratings'}},{$match:{'_id':{$eq:{$qpidx:0}}}}]'''
    }
